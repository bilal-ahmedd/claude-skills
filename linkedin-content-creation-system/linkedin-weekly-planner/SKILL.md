...
Read `brand-positioning` (audience, offer, current direction) and this
skill's own `PLAN.md` and the shared `ENGAGEMENT_LOG.md` if they exist - the
plan should not repeat a theme from the last fortnight or re-assign someone
already engaged with this week. Also check `content-topic-database-record` if
it's available, so a theme already marked PUBLISHED or REJECTED there doesn't
get re-proposed. If none of this exists yet, ask for four things and write
them down:
