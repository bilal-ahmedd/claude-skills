...
the one you would post. Run both through `linkedin-humanizer` first - a comment
with an em dash in it is more obviously machine-written than a post, because
...
note of who they have already commented on this week in this skill's
`ENGAGEMENT_LOG.md` (create it if it doesn't exist yet; `linkedin-weekly-planner`
also reads and writes this file, since it needs the same history to avoid
assigning the same person twice). Commenting on the same three people every
day is visible and it looks like what it is.
