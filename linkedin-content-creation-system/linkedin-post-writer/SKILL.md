...
1. Read `brand-positioning` for identity, audience, and offer, and this
   skill's own `ACCOUNT_HISTORY.md` for voice notes and validated learnings
   from the account's real post history (proof must be fresh not reused,
   client-story structure is the strongest proven pattern, avoid honest-failure
   postmortems and stat-first hooks by default, etc.) — treat these as real
   priors, not a cold start. If the user gives more recent posts or feedback
   that updates the voice, note it, but don't discard what's already here
   without a reason.
...
**3. Humanize it.** Run the draft through `linkedin-humanizer` before showing it. Every
...
append the post to this skill's own `POST_LOG.md` (create it if it doesn't
exist, seeded already with the 15-post `ACCOUNT_HISTORY.md` above) with the
date, the hook used, and the first line, so `linkedin-audit` has a continuous
history to work from later.
...
```
/write a linkedin post: we built an internal tool that cut proposal time from 5 hours to 20 min
```
