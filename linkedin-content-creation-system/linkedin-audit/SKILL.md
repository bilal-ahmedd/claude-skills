...
Also read `linkedin-post-writer/POST_LOG.md` and its seed file `ACCOUNT_HISTORY.md`
if they exist, since they record which hook formula each post used and the
account's 15-post history and validated learnings migrated from the prior
evidence system — start from that, don't treat this as a cold account.
...
- Hook formula. Which numbers from `linkedin-post-writer/hooks.json` are in the top 5?
...
Then hand the conclusions to `linkedin-weekly-planner` so next week's plan is built on the
